...
__device__ __constant__ float d_filter[1024];
...
__global__ void gaussian_blur(
  ...
  for (int filter_r = -radius; filter_r <= radius; filter_r++) {
    for (int filter_c = -radius; filter_c <= radius; filter_c++) {
      ...
      float filter_value =
          d_filter[(filter_r + radius) * filterWidth + filter_c + radius];
      int image_idx = image_r * numCols + image_c;
      float4 pixel;
      pixel.x = static_cast<float>(inputImage[image_idx].x);
      pixel.y = static_cast<float>(inputImage[image_idx].y);
      pixel.z = static_cast<float>(inputImage[image_idx].z);
      result.x += pixel.x * filter_value;
      result.y += pixel.y * filter_value;
      result.z += pixel.z * filter_value;
    }
    int index = r * numCols + c;
    result.w = 255;
    outputImage[index].x = static_cast<unsigned char>(result.x);
    outputImage[index].y = static_cast<unsigned char>(result.y);
    outputImage[index].z = static_cast<unsigned char>(result.z);
    outputImage[index].w = static_cast<unsigned char>(result.w);
  }
  // TODO
...
void allocateMemoryAndCopyToGPU(
  ...
  checkCudaErrors(cudaMemcpyToSymbol(
      d_filter, h_filter, sizeof(float) * filterWidth * filterWidth));
  // TODO:
...
void your_gaussian_blur(
  ...
  const int sz = 32;
  const dim3 blockSize(sz, sz, 1);

  // TODO:
  // Compute correct grid size (i.e., number of blocks per kernel launch)
  // from the image size and and block size.
  const dim3 gridSize(numCols / sz + 1, numRows / sz + 1, 1);

  // TODO: Launch a kernel for separating the RGBA image into different color
  // channels
  gaussian_blur<<<gridSize, blockSize>>>(
      d_inputImageRGBA, d_outputImageRGBA, numRows, numCols, filterWidth);
